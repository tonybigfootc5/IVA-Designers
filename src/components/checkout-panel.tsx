...
export function CheckoutPanel({ courses }: { courses: CourseSummary[] }) {
  ...
  async function submitCheckout() {
    setStatus("Getting your checkout ready...");
    ...
    if (!response.ok) {
      setStatus("Checkout could not start. Please check your details.");
      return;
    }

    const payload = (await response.json()) as CheckoutResponse;
    setResult(payload);
    setStatus(`Checkout is ready with ${payload.checkout.provider}.`);
  }

  return (
    <div className="rounded-[2rem] border border-line bg-card p-6 shadow-glow">
      <p className="text-sm uppercase tracking-[0.24em] text-gold">Join Now</p>
      <h3 className="mt-3 font-display text-3xl text-foreground">Fill your details and continue to payment</h3>
      ...
      <p className="mt-4 text-sm text-muted">{status ?? "Choose a course, enter your details, and move to payment."}</p>
      {result ? (
        <div className="mt-6 rounded-[1.5rem] border border-line bg-white/5 p-4 text-sm text-muted">
          <p>Course fee: <span className="text-foreground">{formatCurrency(result.pricing.subtotal)}</span></p>
          <p>Discount: <span className="text-foreground">{formatCurrency(result.pricing.discount)}</span></p>
          <p>Final amount: <span className="font-semibold text-gold-soft">{formatCurrency(result.pricing.total)}</span></p>
          <p>Payment type: <span className="text-foreground">{result.checkout.mode}</span></p>
          <p>Payment link: <span className="text-foreground">{result.checkout.redirectUrl}</span></p>
        </div>
      ) : null}
    </div>
  );
